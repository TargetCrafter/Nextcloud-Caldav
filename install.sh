#!/usr/bin/env bash
# Installs or upgrades the Nextcloud Caldav plasmoid for the current user.
#
# Usage:
#   ./install.sh                                    # auto-detect (see below)
#   ./install.sh com.vertc.nextcloud.caldav.plasmoid   # install a specific package
set -euo pipefail

PLUGIN_ID="com.vertc.nextcloud.caldav"
# Older installs used the plugin id com.github.targetcrafter.kdecaldav - as
# far as kpackagetool6 is concerned that's a different applet, so switching
# to this id leaves the old one behind rather than upgrading it. Remove it
# manually if it's still installed:
#   kpackagetool6 --type Plasma/Applet --remove com.github.targetcrafter.kdecaldav
...
echo
echo "Done. Add it via: right-click desktop or panel -> Add Widgets... -> search 'Nextcloud Caldav'."
